...
# 生成一个新的测试集，其中的domain是训练集中没有出现过的
...
def Generate_data(same_service_list):
    ...
    #for split in [ "test"]:
        ...
        out = open(os.path.join(data_path_out, "{}_zero-shot.json".format(split)), "w") # W 模式 会覆盖之前的内容
        idx_out = open(os.path.join(data_path_out, "{}_zero-shot.idx".format(split)), "w")
